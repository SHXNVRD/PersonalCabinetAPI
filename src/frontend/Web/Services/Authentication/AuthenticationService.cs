...
public class AuthenticationService
{
    ...
    {
        await _localStorage.RemoveItemsAsync([AccessTokenKey, RefreshTokenKey]);
        await ((JwtAuthStateProvider)_authStateProvider).NotifyUserAuthenticationAsync();
    }
    ...
    public async Task<Result<string>> RefreshTokenAsync()
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            if (string.IsNullOrWhiteSpace(rawContent))
                return Result.Fail("Не удалось обновить токен доступа. Пройдите процедуру авторизации.");
            
            var problemDetails = JsonSerializer.Deserialize<ProblemDetails>(rawContent);
            var message = problemDetails?.Errors.FirstOrDefault().Value[0] ?? "Не удалось обновить токен доступа. Пройдите процедуру авторизации.";
    ...
    private async Task<Result<AuthResponse>> AuthenticateAsync(string password, string login)
    {
        var request = new AuthRequest(password, login);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            var message = problemDetails?.Errors.FirstOrDefault().Value[0] ?? "Не удалось авторизоваться.";
            return Result.Fail(message);
        }
        ...
        return new AuthResponse(result.AccessToken, result.TokenType, result.ExpiresIn, result.RefreshToken);
    }
}