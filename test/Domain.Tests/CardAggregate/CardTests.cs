...
public class CardTests
{
    ...
    private readonly decimal _balance = 10m;
    private readonly Guid _userId = Guid.NewGuid();
    private readonly Category _category = Category.Create("Category").Value;
    private readonly Quantity _one = Quantity.Create(1d).Value;
    ...
    [Fact]
    public void Create_CardBalanceLessZero_ReturnsFail()
    {
        var result = Card.Create(_number, _pinHash, -1m);
    ...
    [Fact]
    public void Buy_PurchaseTotalAmountGreaterThenCardBalance_ReturnsFail()
    {
        var card = Card.Create(_number, _pinHash, _balance).Value;
        card.Activate(_userId);
        var purchase = Purchase.Create(card.Id).Value;
        var product = Product.Create(_productTitle, _productDescription, 100m, _one, _category).Value;
        purchase.AddOrUpdate(product, _one);
    ...
    [Fact]
    public void RefundLatest_SuccessCase_ReturnsFail()
    {
        var card = Card.Create(_number, _pinHash, _balance).Value;
        card.Activate(_userId);
        var purchase = Purchase.Create(card.Id).Value;
        ...
        var result = card.RefundLatest(product.Id, _productPrice, _one);
    ...
    [Fact]
    public void RefundLatest_PurchaseNotFoundForRefund_ReturnsFail()
    {
        var card = Card.Create(_number, _pinHash, _balance).Value;

        var result = card.RefundLatest(1, 1m, _one);
        
        Assert.True(result.IsFailed);
    }
    
    [Fact]
    public void RefundLatest_LatestPurchaseAlreadyRefunded_ReturnsFail()
    {
        var card = Card.Create(_number, _pinHash, _balance).Value;
        card.Activate(_userId);
        var purchase = Purchase.Create(card.Id).Value;
        ...
        purchase.AddOrUpdate(product, _one);
        card.Buy(purchase);
        card.RefundLatest(product.Id, _productPrice, _one);
    ...
    [Fact]
    public void RefundLatest_ProductPriceAtRefundNotMatchProductPriceAtPurchase_ReturnsFail()
    {
        var card = Card.Create(_number, _pinHash, _balance).Value;
        card.Activate(_userId);
        var purchase = Purchase.Create(card.Id).Value;
        ...
        var result = card.RefundLatest(product.Id, 2m, _one);

        Assert.True(result.IsFailed);
    }
    
    [Fact]
    public void RefundLatest_ProductQuantityAtRefundNotMatchProductQuantityAtPurchase_ReturnsFail()
    {
        var card = Card.Create(_number, _pinHash, _balance).Value;
        card.Activate(_userId);
        var purchase = Purchase.Create(card.Id).Value;
        var product = Product.Create(_productTitle, _productDescription, _productPrice, Quantity.Create(1d).Value, _category).Value;
        purchase.AddOrUpdate(product, _one);
        card.Buy(purchase);
        
        var result = card.RefundLatest(product.Id, _productPrice, Quantity.Create(2d).Value);
